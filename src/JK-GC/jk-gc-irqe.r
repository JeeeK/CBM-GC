...
; ******** Source: jk-gc.asm
   ...
   211                          HOOK
   212  c512 8dbfc7             	STA ORIGIRQ
   213  c515 8ec0c7             	STX ORIGIRQ+1
   ...
   347                          
   348  c568 6cbfc7             CONT	JMP (ORIGIRQ)	; weiter zum Original-Code (Kette).
   349                          
   350                          CHK_PC
   ...
   517  c618 bdc0c7             RESLOOP	LDA SAVE-1,X	; Zeropage-Reg.
   518  c61b 954b               	STA ZPSTART-1,X	; restaurieren.
   519  c61d ca                 	DEX
   520  c61e d0f8               	BNE RESLOOP
   521                          
   522  c620 adbdc7             	LDA ORIGVID	; Kontrollanzeige löschen
   523  c623 8de707             	STA MARKEVID	; und alten Zustand wieder
   524  c626 adbec7             	LDA ORIGCOL	; herstellen.
   ...
   542  c62d a900               	LDA #0		; Buffer-Pointer auf
   543  c62f 855f               	STA BUFPTR	; $10000 (65536) = 0
   544  c631 8560               	STA BUFPTR+1	; setzen.
   545                          }
   546  c633 38                 	SEC
   547  c634 24                 	!byte $24	; BIT ZP, d.h. nächsten Befehl ignorieren!
   548                          NEXTSTR	
   549  c635 18                 	CLC
   550                          NEXTSTR1
   551  c636 20a0c6             	JSR GETSA	; Nächste String-Adresse holen.
   552  c639 f03b               	BEQ COPYBACK	; keinen String mehr gefunden!
   553                          
   554  c63b 98                 	TYA		; high Byte
   555  c63c e45d               	CPX RNGEND	; X/A >= RNGEND:
   556  c63e e55e               	SBC RNGEND+1	; oberhalb des Bereichs, dann
   557  c640 b0f3               	BCS NEXTSTR	; nächster String!
   558                          
   559  c642 98                 	TYA		; high Byte
   560  c643 e44c               	CPX RNGBEG	; X/A < RNGBEG:
   561  c645 e54d               	SBC RNGBEG+1	; unterhalb des Bereichs, dann
   562  c647 90ed               	BCC NEXTSTR1	; nächster String!
   563                          			; Innerhalb des Bereichs:
   564  c649 a55f               	LDA BUFPTR	; Pufferzeiger um
   565  c64b e552               	SBC LEN		; String-Länge nach unten
   566  c64d 855f               	STA BUFPTR	; setzen.
   567  c64f b002               	BCS +
   568  c651 c660               	DEC BUFPTR+1	; Überlauf High-Byte
   569                          
   570  c653 8459               +	STY STRADR+1	; String-Adresse abspeichern
   571  c655 8658               	STX STRADR	; für Kopieraktion.
   572                          
   573  c657 a452               	LDY LEN		; String-Länge (> 0)
   574  c659 d004               	BNE NBENTRY	; immer, mit Dekrement beginnen!
   575  c65b b158               NEXTBYT	LDA (STRADR),Y	; String in den Pufferbereich
   576  c65d 915f               	STA (BUFPTR),Y	; übertragen, ROM ist aktiv
   577  c65f 88                 NBENTRY	DEY		; schreibt ins RAM unters ROM!
   578  c660 d0f9               	BNE NEXTBYT
   579                          LEN1
   580  c662 b158               	LDA (STRADR),Y	; Das 0. Byte extra
   581  c664 915f               	STA (BUFPTR),Y	; übertragen
   582                          
   583  c666 38                 	SEC		; Neue String-Adresse:
   584  c667 a54e               	LDA NEWPTR	; Einfach den Pointer
   585  c669 e552               	SBC LEN		; mitziehen, ebenso um
   586  c66b 854e               	STA NEWPTR	; String-Länge nach unten
   587  c66d b002               	BCS +		; setzen.
   588  c66f c64f               	DEC NEWPTR+1	; Überlauf High-Byte
   589                          +
   590  c671 2078c7             	JSR CORR	; String-Adresse in Descriptor ändern.
   591                          			; Immmer Z=0,
   592  c674 d0bf               	BNE NEXTSTR	; zum nächsten String.
   593                          
   594                          
   ...
   617  c676 a55f               	LDA BUFPTR	; Puffer leer,
   618  c678 0560               	ORA BUFPTR+1	; wenn Pointer =0 (Ende)
   619  c67a f021               	BEQ NOCOPY	; War es letzter Bereich?
   620                          }
   621                          
   ...
   625                          } else {
   626  c67c a54e               	LDA NEWPTR	; Optimiertes MOVBLOCK braucht
   627  c67e a64f               	LDX NEWPTR+1	; Zielblockanfang	
   628                          }
   629  c680 8558               	STA $58		; = STRADR
   630  c682 8659               	STX $59		; je nach MOVBLOCK-Variante
   ...
   635                          	LDX NEWPTR+1	
   636                          }
   637  c684 8533               	STA STRPTR	; neues FRETOP
   638  c686 8634               	STX STRPTR+1
   639                          
   ...
   647  c688 a900               	LDA #$00	; Quellblockende+1
   648  c68a 855a               	STA $5A
   649  c68c 855b               	STA $5B
   650                          }
   651                          			; Quellbockanfang = BUFPTR
   652                          
   653  c68e 78                 	SEI		; keine Interrupts zulassen, wegen
   654  c68f a501               	LDA PROCPORT	; KERNAL-ROM wegblenden
   655  c691 48                 	PHA		; damit das Puffer-RAM zugänglich
   656  c692 a935               	LDA #MEMRAM	; wird. Auch BASIC-ROM ist damit weg!
   657                          			; Sowohl bei Puffer $F000 als auch $A000
   658                          			; werden beide ROMs ausgeblendet.
   659  c694 8501               	STA PROCPORT
   660                          
   661  c696 2087c7             	JSR MOVBLOCK	; BASIC-Routine Blockverschieben
   ...
   665                          			; Z=1
   666  c699 68                 	PLA		; ursprünglicher Zustand
   667  c69a 8501               	STA PROCPORT	; KERNAL-ROM wieder aktivieren
   668  c69c 58                 	CLI
   669                          NOCOPY
   670  c69d 4ce7c5             	JMP NEXTBLOCK	; nächsten Bereich
   671                          
   672                          
   673                          ;
   ...
   682                          ; die Stringlänge.
   683                          
   684  c6a0 9063               GETSA	BCC CHECKTYPE	; C=0 -> nächsten String laut STAT
   ...
   699                          DESCSTACK
   700  c6a2 a000               	LDY #0
   701  c6a4 8423               	STY STRDP+1	; Descriptor auf
   702  c6a6 a905               	LDA #STAT_SDS	; Status: SDS
   703  c6a8 8557               	STA STAT
   704  c6aa a219               	LDX #TOSS	; SDS Start
   705  c6ac d005               	BNE ISDSTEND	; immer verzweigen
   706  c6ae a622               DSTACK	LDX STRDP
   707  c6b0 e8                 NEXTDST	INX		; nächster Descriptor
   708  c6b1 e8                 	INX
   709  c6b2 e8                 	INX
   710                          ISDSTEND
   711  c6b3 e416               	CPX TSSP	; Stack durch?
   712  c6b5 f010               	BEQ VARS
   713  c6b7 b500               	LDA 0,X		; String-Länge
   714  c6b9 f0f5               	BEQ NEXTDST
   715  c6bb 8552               	STA LEN		; Rückgabevariable
   716  c6bd 8622               	STX STRDP	; festhalten
   717  c6bf b502               	LDA 2,X		; String-Adr. high
   718  c6c1 a8                 	TAY
   719  c6c2 b501               	LDA 1,X		; String-Adr. low
   720  c6c4 aa                 	TAX
   721  c6c5 98                 	TYA		; immer ungleich 0, Z=0
   722  c6c6 60                 	RTS		; Adresse in X/Y retour
   723                          
   724                          ; *** Variablen: VARTAB bis ARYTAB
   725                          
   726  c6c7 a52d               VARS	LDA VARTAB	; Variablenanfang
   727  c6c9 a62e               	LDX VARTAB+1
   728  c6cb 8522               	STA STRDP
   729  c6cd 8623               	STX STRDP+1
   730  c6cf a003               	LDY #STAT_VAR	; Status: einfache Variablen
   731  c6d1 8457               	STY STAT
   732  c6d3 d00b               	BNE ISVAREND	; immer verzweigen
   733                          VAR
   734  c6d5 18                 NEXTVAR	CLC		; nächste Variable
   735  c6d6 a522               	LDA STRDP
   736  c6d8 6907               	ADC #7		; Variablenlänge
   737  c6da 8522               	STA STRDP
   738  c6dc 9002               	BCC ISVAREND
   739  c6de e623               	INC STRDP+1	; Überlauf High-Byte
   740                          ISVAREND
   741  c6e0 c52f               	CMP ARYTAB
   742  c6e2 d006               	BNE CHECKVAR
   743  c6e4 a623               	LDX STRDP+1	; Var.-Ende (=Array-Anfang)?
   744  c6e6 e430               	CPX ARYTAB+1
   745  c6e8 f026               	BEQ ARRAYS	; Var.-Ende, weiter mit Arrays
   746                          CHECKVAR
   747  c6ea a000               	LDY #0		; Variablenname
   748  c6ec b122               	LDA (STRDP),Y	; 1. Zeichen, Typ in Bit 7 
   749  c6ee 30e5               	BMI NEXTVAR	; kein String, nächste V.
   750  c6f0 c8                 	INY
   751  c6f1 b122               	LDA (STRDP),Y	; 2. Zeichen, Typ in Bit 7
   752  c6f3 10e0               	BPL NEXTVAR	; kein String, nächste V.
   753  c6f5 c8                 	INY
   754  c6f6 b122               	LDA (STRDP),Y	; String-Länge
   755  c6f8 f0db               	BEQ NEXTVAR	; = 0, Nächste Variable
   756  c6fa 8552               	STA LEN		; Rückgabevariable
   757  c6fc c8                 	INY
   758  c6fd b122               	LDA (STRDP),Y	; String-Adresse low
   759  c6ff aa                 	TAX
   760  c700 c8                 	INY
   761  c701 b122               	LDA (STRDP),Y	; String-Adresse high
   762  c703 a8                 	TAY		; immer ungleich 0, Z=0
   763  c704 60                 	RTS		; Adresse in X/Y retour
   764                          
   765                          CHECKTYPE
   766  c705 a557               	LDA STAT	; GETSA-Einstieg mit C=0
   767  c707 c903               	CMP #STAT_VAR	; String-Status?
   768  c709 9042               	BCC ARRAY	; =1 -> Arrays
   769  c70b f0c8               	BEQ VAR		; =3 -> Variablen
   770  c70d 4caec6             	JMP DSTACK	; =5 -> String-Desc.-Stack
   771                          
   772                          ; *** Arrays: ARYTAB bis STREND
   773                          
   774  c710 8550               ARRAYS	STA PTR		; A/X von Variablendurchlauf
   775  c712 8651               	STX PTR+1	; Start Array-Array-Bereich
   776  c714 a001               	LDY #STAT_ARY
   777  c716 8457               	STY STAT	; Status: Arrays
   778                          ISARREND
   779  c718 a550               	LDA PTR
   780  c71a a651               	LDX PTR+1
   781  c71c e432               CHKAEND	CPX STREND+1	; Ende des Array-Bereichs
   782  c71e d004                       BNE NEXTARR	; erreicht?
   783  c720 c531               	CMP STREND
   784  c722 f04f               	BEQ NOSTRING	; Arrays fertig -> kein String
   785                          NEXTARR
   786  c724 8522               	STA STRDP	; immer C=0
   787  c726 8623               	STX STRDP+1
   788  c728 a000               	LDY #0
   789  c72a b122               	LDA (STRDP),Y	; Array-Name
   790  c72c aa                 	TAX		; Array-Typ merken
   791  c72d c8                 	INY
   792  c72e b122               	LDA (STRDP),Y
   793  c730 08                 	PHP		; Array-Typ merken
   794  c731 c8                 	INY
   795  c732 b122               	LDA (STRDP),Y	; Offset nächstes Array
   796  c734 6550               	ADC PTR		; C-Flag ist bereits 0 (CMP/CPX)
   797  c736 8550               	STA PTR		; Start Folge-Array
   798  c738 c8                 	INY
   799  c739 b122               	LDA (STRDP),Y
   800  c73b 6551               	ADC PTR+1
   801  c73d 8551               	STA PTR+1
   802  c73f 28                 	PLP		; Var.-Typ holen
   803  c740 10d6               	BPL ISARREND	; kein String-Array
   804  c742 8a                 	TXA		; Var.-Typ holen
   805  c743 30d3               	BMI ISARREND	; kein String-Array
   806  c745 c8                 	INY
   807  c746 b122               	LDA (STRDP),Y	; Anzahl der Dimensionen
   808  c748 0a                 	ASL		; *2
   809  c749 6905               	ADC #5		; Offset = Dimensionen*2+5
   810                          			; C=0 solange Dim.. <= 125
   811  c74b d003               	BNE ADVDESC	; immer verzweigen
   812                          ARRAY			; Einstieg bei Fortsetzung
   813                          NEXTASTR
   814  c74d 18                 	CLC
   815  c74e a903               	LDA #3		; String-Descriptor-Länge
   816  c750 6522               ADVDESC	ADC STRDP	; nächten String
   817  c752 8522               	STA STRDP
   818  c754 9002               	BCC +
   819  c756 e623               	INC STRDP+1	; Überlauf High-Byte
   820  c758 c550               +	CMP PTR		; Array durch?
   821  c75a d006               	BNE IS0ASTR
   822  c75c a623               	LDX STRDP+1
   823  c75e e451               	CPX PTR+1
   824  c760 f0ba               	BEQ CHKAEND	; A/X = PTR, Array-Ende prüfen
   825                          IS0ASTR
   826  c762 a000               	LDY #0
   827  c764 b122               	LDA (STRDP),Y	; String-Länge
   828  c766 f0e5               	BEQ NEXTASTR	; weiter im Array
   829  c768 8552               	STA LEN		; Rückgabevariable
   830  c76a c8                 	INY
   831  c76b b122               	LDA (STRDP),Y	; String-Adresse low
   832  c76d aa                 	TAX
   833  c76e c8                 	INY
   834  c76f b122               	LDA (STRDP),Y	; String-Adresse high
   835  c771 a8                 	TAY		; immer ungleich 0, Z=0
   836  c772 60                 	RTS		; Adresse in X/Y retour
   837                          
   838                          NOSTRING
   839  c773 a900               	LDA #0		; Länge 0 
   840  c775 8552               	STA LEN		; kein String gefunden
   841  c777 60                 	RTS		; Z=1
   842                          
   843                          ;
   ...
   846                          ; ( STRADR, STAT -> )
   847                          ;
   848  c778 a557               CORR	LDA STAT	; String-Status
   849  c77a 2903               	AND #%011	; nur 2 Bits
   850  c77c a8                 	TAY		; Lage des Descriptors
   851  c77d a54e               	LDA NEWPTR	;
   852  c77f 9122               	STA (STRDP),Y	; ... bei SDS
   853  c781 c8                 	INY		; ... und Array verschieden!
   854  c782 a54f               	LDA NEWPTR+1
   855  c784 9122               	STA (STRDP),Y
   856  c786 60                 	RTS
   857                          
   858                          
   ...
   881                          MOVBLOCK
   882  c787 38                         SEC
   883  c788 a55a                       LDA $5A       ; Quelle Endadresse low
   884  c78a e55f                       SBC $5F       ; minus Quelle Startadresse low
   885  c78c a8                         TAY           ; Länge low
   886  c78d a55b                       LDA $5B       ; Endadresse high
   887  c78f e560                       SBC $60       ; minus Startadresse high
   888  c791 aa                         TAX           ; Länge high, als DEC-DEC-Counter
   889  c792 98                         TYA           ; Länge low
   890  c793 f019                       BEQ copy      ; wenn 0, dann geht es einfach ...
   891  c795 18                         CLC           ; Länge in A
   892  c796 655f                       ADC $5F       ; Quellstartadresse um Low-Byte-Offset
   893  c798 855f                       STA $5F       ; korrigieren: -(-Länge) -> +Länge
   894  c79a b002                       BCS +         ; da eigentlich Subtraktion, Überlauf
   895  c79c c660                       DEC $60       ; entspr. in Startadresse high korr.
   896                          +
   897  c79e 98                         TYA           ; Länge low
   898  c79f 18                         CLC
   899  c7a0 6558                       ADC $58       ; Zielstartadresse um Low-Bye-Offset
   900  c7a2 8558                       STA $58       ; korrigieren: -(-Länge) -> +Länge
   901  c7a4 b002                       BCS +         ; da eigentlich Subtraktion, Überlauf
   902  c7a6 c659                       DEC $59       ; entspr. in Startadresse high korr.
   903                          +
   904  c7a8 e8                 	INX           ; Page-Zähler (alle ganzen und einen teilweise)
   905  c7a9 98                 	TYA           ; Länge low
   906  c7aa 49ff               	EOR #$FF      ; negieren (2er-Komplement):
   907  c7ac a8                 	TAY           ; NOT(X)+1
   908  c7ad c8                 	INY
   909  c7ae b15f               copy    LDA ($5F),Y   ; Quelle auf
   910  c7b0 9158                       STA ($58),Y   ; Ziel kopieren, aufsteigend
   911  c7b2 c8                         INY
   912  c7b3 d0f9                       BNE copy
   913  c7b5 e660                       INC $60       ; Quelle high
   914  c7b7 e659                       INC $59       ; Ziel high
   915  c7b9 ca                         DEX           ; Blockzähler 
   916  c7ba d0f2                       BNE copy
   917  c7bc 60                         RTS
   ...
   982                          }
   983                          
   984  c7bd 00                 ORIGVID !byte 0		; originale Video der Markenposition
   985  c7be 00                 ORIGCOL !byte 0		; originale Farbe der Markenposition
   986                          !ifndef basic_patch {
   987  c7bf 0000               ORIGIRQ !byte 0,0	; originaler IRQ-Vektor
   988                          }
   989  c7c1 00                 SAVE	!byte 0		; gesicherte ZP-Variablen
   990                          *=*+ZPLEN-1
   991                          
   992                          
