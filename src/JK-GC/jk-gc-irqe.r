...
; ******** Source: jk-gc.asm
   ...
   153                          	LDX #>($2000)	; BASIC-ROM Länge in Pages
   154                          CPYROM	LDA (CPTR),Y	; ROM lesen
   155                          	STA (CPTR),Y	; RAM schreiben
   156                          	INY
   157                          	BNE CPYROM
   158                          	INC CPTR+1	; nächste Page
   159                          	DEX		; Page-Zähler
   160                          	BNE CPYROM
   ...
   179                          HOOK
   180  c50f 8dbcc7             	STA ORIGIRQ
   181  c512 8ebdc7             	STX ORIGIRQ+1
   182  c515 a921               	LDA #<(IRQ)	; GC-Routine einhängen ...
   ...
   275  c53b d028               	BNE CONT	; immer, mit RTI zurück
   276                          
   277                          CORR_STR
   278  c53d a455               	LDY $55		; Offset des Descriptors
   279  c53f c8                 	INY		; String-Länge
   ...
   310                          
   311  c565 6cbcc7             CONT	JMP (ORIGIRQ)	; weiter zum Original-Code (Kette).
   312                          
   313                          CHK_PC
   ...
   354  c59c b008               	BCS +		; darüber, keine Korrektur
   355                          
   ...
   375  c5b2 68                 	PLA		; kommt direkt von RTI, Aufruf-PC (für RTS) in alter GC
   376  c5b3 68                 	PLA		; verwerfen und die neue GC übernimmt ...
   377                          			; direkt durch zu COLLECT
   378                          START_COLLECT
   379  c5b4 a903               	LDA #3
   380  c5b6 8553               	STA $53		; Step-Size für nächsten Descriptor auf
   ...
   471  c615 bdbdc7             RESLOOP	LDA SAVE-1,X	; Zeropage-Reg.
   472  c618 954b               	STA ZPSTART-1,X	; restaurieren.
   473  c61a ca                 	DEX
   474  c61b d0f8               	BNE RESLOOP
   475                          
   476  c61d adbac7             	LDA ORIGVID	; Kontrollanzeige löschen
   477  c620 8de707             	STA MARKEVID	; und alten Zustand wieder
   478  c623 adbbc7             	LDA ORIGCOL	; herstellen.
   ...
   496  c62a a900               	LDA #0		; Buffer-Pointer auf
   497  c62c 855f               	STA BUFPTR	; $10000 (65536) = 0
   498  c62e 8560               	STA BUFPTR+1	; setzen.
   499                          }
   500  c630 38                 	SEC
   501  c631 24                 	!byte $24	; BIT ZP, d.h. nächsten Befehl ignorieren!
   502                          NEXTSTR	
   503  c632 18                 	CLC
   504                          NEXTSTR1
   505  c633 209dc6             	JSR GETSA	; Nächste String-Adresse holen.
   506  c636 f03b               	BEQ COPYBACK	; keinen String mehr gefunden!
   507                          
   508  c638 98                 	TYA		; high Byte
   509  c639 e45d               	CPX BEREND	; X/A >= BEREND:
   510  c63b e55e               	SBC BEREND+1	; oberhalb des Bereichs, dann
   511  c63d b0f3               	BCS NEXTSTR	; nächster String!
   512                          
   513  c63f 98                 	TYA		; high Byte
   514  c640 e44c               	CPX BERANF	; X/A < BERANF:
   515  c642 e54d               	SBC BERANF+1	; unterhalb des Bereichs, dann
   516  c644 90ed               	BCC NEXTSTR1	; nächster String!
   517                          			; Innerhalb des Bereichs:
   518  c646 a55f               	LDA BUFPTR	; Pufferzeiger um
   519  c648 e552               	SBC LEN		; String-Länge nach unten
   520  c64a 855f               	STA BUFPTR	; setzen.
   521  c64c b002               	BCS +
   522  c64e c660               	DEC BUFPTR+1	; Überlauf High-Byte
   523                          
   524  c650 8459               +	STY STRADR+1	; String-Adresse abspeichern
   525  c652 8658               	STX STRADR	; für Kopieraktion.
   526                          
   527  c654 a452               	LDY LEN		; String-Länge (> 0)
   528  c656 d004               	BNE NBENTRY	; immer, mit Dekrement beginnen!
   529  c658 b158               NEXTBYT	LDA (STRADR),Y	; String in den Pufferbereich
   530  c65a 915f               	STA (BUFPTR),Y	; übertragen, ROM ist aktiv
   531  c65c 88                 NBENTRY	DEY		; schreibt ins RAM unters ROM!
   532  c65d d0f9               	BNE NEXTBYT
   533                          LEN1
   534  c65f b158               	LDA (STRADR),Y	; Das 0. Byte extra
   535  c661 915f               	STA (BUFPTR),Y	; übertragen
   536                          
   537  c663 38                 	SEC		; Neue String-Adresse:
   538  c664 a54e               	LDA NEWPTR	; Einfach den Pointer
   539  c666 e552               	SBC LEN		; mitziehen, ebenso um
   540  c668 854e               	STA NEWPTR	; String-Länge nach unten
   541  c66a b002               	BCS +		; setzen.
   542  c66c c64f               	DEC NEWPTR+1	; Überlauf High-Byte
   543                          +
   544  c66e 2075c7             	JSR CORR	; String-Adresse in Descriptor ändern.
   545                          			; Immmer Z=0,
   546  c671 d0bf               	BNE NEXTSTR	; zum nächsten String.
   547                          
   548                          
   ...
   571  c673 a55f               	LDA BUFPTR	; Puffer leer,
   572  c675 0560               	ORA BUFPTR+1	; wenn Pointer =0 (Ende)
   573  c677 f021               	BEQ NOCOPY	; War es letzter Bereich?
   574                          }
   575                          
   ...
   579                          } else {
   580  c679 a54e               	LDA NEWPTR	; Optimiertes MOVBLOCK braucht
   581  c67b a64f               	LDX NEWPTR+1	; Zielblockanfang	
   582                          }
   583  c67d 8558               	STA $58		; = STRADR
   584  c67f 8659               	STX $59
   585                          
   586                          !ifdef orig_movblock {
   587                          	LDA NEWPTR
   588                          	LDX NEWPTR+1	
   589                          }
   590  c681 8533               	STA STRPTR	; neues FRETOP
   591  c683 8634               	STX STRPTR+1
   592                          
   ...
   600  c685 a900               	LDA #$00	; Quellblockende+1
   601  c687 855a               	STA $5A
   602  c689 855b               	STA $5B
   603                          }
   604                          			; Quellbockanfang = BUFPTR
   605                          
   606  c68b 78                 	SEI		; keine Interrupts zulassen, wegen
   607  c68c a501               	LDA PROZPORT	; KERNAL-ROM wegblenden
   608  c68e 48                 	PHA		; damit das Puffer-RAM zugänglich
   609  c68f a935               	LDA #MEMRAM	; wird. Auch BASIC-ROM ist damit weg!
   610  c691 8501               	STA PROZPORT
   611                          
   612  c693 2084c7             	JSR MOVBLOCK	; BASIC-Routine Blockverschieben
   ...
   616                          			; Z=1
   617  c696 68                 	PLA		; ursprünglicher Zustand
   618  c697 8501               	STA PROZPORT	; KERNAL-ROM wieder aktivieren
   619  c699 58                 	CLI
   620                          NOCOPY
   621  c69a 4ce4c5             	JMP NEXTBLOCK	; nächsten Bereich
   622                          
   623                          
   624                          ;
   ...
   633                          ; die Stringlänge.
   634                          
   635  c69d 9063               GETSA	BCC CHECKTYPE	; C=0 -> nächsten String laut STAT
   ...
   650                          DESCSTACK
   651  c69f a000               	LDY #0
   652  c6a1 8423               	STY STRDP+1	; Descriptor auf
   653  c6a3 a905               	LDA #STAT_SDS	; Status: SDS
   654  c6a5 8557               	STA STAT
   655  c6a7 a219               	LDX #TOSS	; SDS Start
   656  c6a9 d005               	BNE ISDSTEND	; immer verzweigen
   657  c6ab a622               DSTACK	LDX STRDP
   658  c6ad e8                 NEXTDST	INX		; nächster Descriptor
   659  c6ae e8                 	INX
   660  c6af e8                 	INX
   661                          ISDSTEND
   662  c6b0 e416               	CPX TSSP	; Stack durch?
   663  c6b2 f010               	BEQ VARS
   664  c6b4 b500               	LDA 0,X		; String-Länge
   665  c6b6 f0f5               	BEQ NEXTDST
   666  c6b8 8552               	STA LEN		; Rückgabevariable
   667  c6ba 8622               	STX STRDP	; festhalten
   668  c6bc b502               	LDA 2,X		; String-Adr. high
   669  c6be a8                 	TAY
   670  c6bf b501               	LDA 1,X		; String-Adr. low
   671  c6c1 aa                 	TAX
   672  c6c2 98                 	TYA		; immer ungleich 0, Z=0
   673  c6c3 60                 	RTS		; Adresse in X/Y retour
   674                          
   675                          ; *** Variablen: VARTAB bis ARYTAB
   676                          
   677  c6c4 a52d               VARS	LDA VARTAB	; Variablenanfang
   678  c6c6 a62e               	LDX VARTAB+1
   679  c6c8 8522               	STA STRDP
   680  c6ca 8623               	STX STRDP+1
   681  c6cc a003               	LDY #STAT_VAR	; Status: einfache Variablen
   682  c6ce 8457               	STY STAT
   683  c6d0 d00b               	BNE ISVAREND	; immer verzweigen
   684                          VAR
   685  c6d2 18                 NEXTVAR	CLC		; nächste Variable
   686  c6d3 a522               	LDA STRDP
   687  c6d5 6907               	ADC #7		; Variablenlänge
   688  c6d7 8522               	STA STRDP
   689  c6d9 9002               	BCC ISVAREND
   690  c6db e623               	INC STRDP+1	; Überlauf High-Byte
   691                          ISVAREND
   692  c6dd c52f               	CMP ARYTAB
   693  c6df d006               	BNE CHECKVAR
   694  c6e1 a623               	LDX STRDP+1	; Var.-Ende (=Array-Anfang)?
   695  c6e3 e430               	CPX ARYTAB+1
   696  c6e5 f026               	BEQ ARRAYS	; Var.-Ende, weiter mit Arrays
   697                          CHECKVAR
   698  c6e7 a000               	LDY #0		; Variablenname
   699  c6e9 b122               	LDA (STRDP),Y	; 1. Zeichen, Typ in Bit 7 
   700  c6eb 30e5               	BMI NEXTVAR	; kein String, nächste V.
   701  c6ed c8                 	INY
   702  c6ee b122               	LDA (STRDP),Y	; 2. Zeichen, Typ in Bit 7
   703  c6f0 10e0               	BPL NEXTVAR	; kein String, nächste V.
   704  c6f2 c8                 	INY
   705  c6f3 b122               	LDA (STRDP),Y	; String-Länge
   706  c6f5 f0db               	BEQ NEXTVAR	; = 0, Nächste Variable
   707  c6f7 8552               	STA LEN		; Rückgabevariable
   708  c6f9 c8                 	INY
   709  c6fa b122               	LDA (STRDP),Y	; String-Adresse low
   710  c6fc aa                 	TAX
   711  c6fd c8                 	INY
   712  c6fe b122               	LDA (STRDP),Y	; String-Adresse high
   713  c700 a8                 	TAY		; immer ungleich 0, Z=0
   714  c701 60                 	RTS		; Adresse in X/Y retour
   715                          
   716                          CHECKTYPE
   717  c702 a557               	LDA STAT	; GETSA-Einstieg mit C=0
   718  c704 c903               	CMP #STAT_VAR	; String-Status?
   719  c706 9042               	BCC ARRAY	; =1 -> Arrays
   720  c708 f0c8               	BEQ VAR		; =3 -> Variablen
   721  c70a 4cabc6             	JMP DSTACK	; =5 -> String-Desc.-Stack
   722                          
   723                          ; *** Arrays: ARYTAB bis STREND
   724                          
   725  c70d 8550               ARRAYS	STA PTR		; A/X von Variablendurchlauf
   726  c70f 8651               	STX PTR+1	; Start Array-Array-Bereich
   727  c711 a001               	LDY #STAT_ARY
   728  c713 8457               	STY STAT	; Status: Arrays
   729                          ISARREND
   730  c715 a550               	LDA PTR
   731  c717 a651               	LDX PTR+1
   732  c719 e432               CHKAEND	CPX STREND+1	; Ende des Array-Bereichs
   733  c71b d004                       BNE NEXTARR	; erreicht?
   734  c71d c531               	CMP STREND
   735  c71f f04f               	BEQ NOSTRING	; Arrays fertig -> kein String
   736                          NEXTARR
   737  c721 8522               	STA STRDP	; immer C=0
   738  c723 8623               	STX STRDP+1
   739  c725 a000               	LDY #0
   740  c727 b122               	LDA (STRDP),Y	; Array-Name
   741  c729 aa                 	TAX		; Array-Typ merken
   742  c72a c8                 	INY
   743  c72b b122               	LDA (STRDP),Y
   744  c72d 08                 	PHP		; Array-Typ merken
   745  c72e c8                 	INY
   746  c72f b122               	LDA (STRDP),Y	; Offset nächstes Array
   747  c731 6550               	ADC PTR		; C-Flag ist bereits 0 (CMP/CPX)
   748  c733 8550               	STA PTR		; Start Folge-Array
   749  c735 c8                 	INY
   750  c736 b122               	LDA (STRDP),Y
   751  c738 6551               	ADC PTR+1
   752  c73a 8551               	STA PTR+1
   753  c73c 28                 	PLP		; Var.-Typ holen
   754  c73d 10d6               	BPL ISARREND	; kein String-Array
   755  c73f 8a                 	TXA		; Var.-Typ holen
   756  c740 30d3               	BMI ISARREND	; kein String-Array
   757  c742 c8                 	INY
   758  c743 b122               	LDA (STRDP),Y	; Anzahl der Dimensionen
   759  c745 0a                 	ASL		; *2
   760  c746 6905               	ADC #5		; Offset = Dimensionen*2+5
   761                          			; C=0 solange Dim.. <= 125
   762  c748 d003               	BNE ADVDESC	; immer verzweigen
   763                          ARRAY			; Einstieg bei Fortsetzung
   764                          NEXTASTR
   765  c74a 18                 	CLC
   766  c74b a903               	LDA #3		; String-Descriptor-Länge
   767  c74d 6522               ADVDESC	ADC STRDP	; nächten String
   768  c74f 8522               	STA STRDP
   769  c751 9002               	BCC +
   770  c753 e623               	INC STRDP+1	; Überlauf High-Byte
   771  c755 c550               +	CMP PTR		; Array durch?
   772  c757 d006               	BNE IS0ASTR
   773  c759 a623               	LDX STRDP+1
   774  c75b e451               	CPX PTR+1
   775  c75d f0ba               	BEQ CHKAEND	; A/X = PTR, Array-Ende prüfen
   776                          IS0ASTR
   777  c75f a000               	LDY #0
   778  c761 b122               	LDA (STRDP),Y	; String-Länge
   779  c763 f0e5               	BEQ NEXTASTR	; weiter im Array
   780  c765 8552               	STA LEN		; Rückgabevariable
   781  c767 c8                 	INY
   782  c768 b122               	LDA (STRDP),Y	; String-Adresse low
   783  c76a aa                 	TAX
   784  c76b c8                 	INY
   785  c76c b122               	LDA (STRDP),Y	; String-Adresse high
   786  c76e a8                 	TAY		; immer ungleich 0, Z=0
   787  c76f 60                 	RTS		; Adresse in X/Y retour
   788                          
   789                          NOSTRING
   790  c770 a900               	LDA #0		; Länge 0 
   791  c772 8552               	STA LEN		; kein String gefunden
   792  c774 60                 	RTS		; Z=1
   793                          
   794                          ;
   ...
   797                          ; ( STRADR, STAT -> )
   798                          ;
   799  c775 a557               CORR	LDA STAT	; String-Status
   800  c777 2903               	AND #%011	; nur 2 Bits
   801  c779 a8                 	TAY		; Lage des Descriptors
   802  c77a a54e               	LDA NEWPTR	;
   803  c77c 9122               	STA (STRDP),Y	; ... bei SDS
   804  c77e c8                 	INY		; ... und Array verschieden!
   805  c77f a54f               	LDA NEWPTR+1
   806  c781 9122               	STA (STRDP),Y
   807  c783 60                 	RTS
   808                          
   ...
   829                          MOVBLOCK
   830  c784 38                         SEC
   831  c785 a55a                       LDA $5A       ; Quelle Endadresse low
   832  c787 e55f                       SBC $5F       ; minus Quelle Startadresse low
   833  c789 a8                         TAY           ; Länge low
   834  c78a a55b                       LDA $5B       ; Endadresse high
   835  c78c e560                       SBC $60       ; minus Startadresse high
   836  c78e aa                         TAX           ; Länge high, als DEC-DEC-Counter
   837  c78f 98                         TYA           ; Länge low
   838  c790 f019                       BEQ copy      ; wenn 0, dann geht es einfach ...
   839  c792 18                         CLC           ; Länge in A
   840  c793 655f                       ADC $5F       ; Quellstartadresse um Low-Byte-Offset
   841  c795 855f                       STA $5F       ; korrigieren: -(-Länge) -> +Länge
   842  c797 b002                       BCS +         ; da eigentlich Subtraktion, Überlauf
   843  c799 c660                       DEC $60       ; entspr. in Startadresse high korr.
   844                          +
   845  c79b 98                         TYA           ; Länge low
   846  c79c 18                         CLC
   847  c79d 6558                       ADC $58       ; Zielstartadresse um Low-Bye-Offset
   848  c79f 8558                       STA $58       ; korrigieren: -(-Länge) -> +Länge
   849  c7a1 b002                       BCS +         ; da eigentlich Subtraktion, Überlauf
   850  c7a3 c659                       DEC $59       ; entspr. in Startadresse high korr.
   851                          +
   852  c7a5 e8                 	INX           ; Page-Zähler (alle ganzen und einen teilweise)
   853  c7a6 98                 	TYA           ; Länge low
   854  c7a7 49ff               	EOR #$FF      ; negieren (2er-Komplement):
   855  c7a9 a8                 	TAY           ; NOT(X)+1
   856  c7aa c8                 	INY
   857  c7ab b15f               copy    LDA ($5F),Y   ; Quelle auf
   858  c7ad 9158                       STA ($58),Y   ; Ziel kopieren, aufsteigend
   859  c7af c8                         INY
   860  c7b0 d0f9                       BNE copy
   861  c7b2 e660                       INC $60       ; Quelle high
   862  c7b4 e659                       INC $59       ; Ziel high
   863  c7b6 ca                         DEX           ; Blockzähler 
   864  c7b7 d0f2                       BNE copy
   865  c7b9 60                         RTS
   ...
   928                          }
   929                          
   930  c7ba 00                 ORIGVID !byte 0		; originale Video der Markenposition
   931  c7bb 00                 ORIGCOL !byte 0		; originale Farbe der Markenposition
   932                          !ifndef basic_patch {
   933  c7bc 0000               ORIGIRQ !byte 0,0	; originaler IRQ-Vektor
   934                          }
   935  c7be 00                 SAVE	!byte 0		; gesicherte ZP-Variablen
   936                          *=*+ZPLEN-1
   937                          
   938                          
