...
L2	INC FLAG	; JA, ENDEFLAG SETZEN
	...
	STA BERANF	; BEREICHSANFANG = BEREICHENDE
	STX BERANF+1	; (SONDERFALL)
	CMP BEREND	; -> BEREICH HAT 0 BYTE LANG
	BNE L1		; -> FERTIG
	CPX BEREND+1
	BEQ ENDE
...
BEGIN2	JSR GETSA	; NAECHSTE STRING ADRESSE HOLEN
	LDA LEN		; WENN 0, DANN
	BEQ WEITER	; KEIN STRING MEHR
	TYA		; GEFUNDEN!
	CPX BERANF	;
	SBC BERANF+1
	...
	BCS L3		; NAECHSTER STRING!
	SEC
	LDA BUFPTR	; STRING LAENGE VERSCHIEBEN
	SBC LEN
	STA BUFPTR
	BCS L4
	DEC BUFPTR+1
...
LOOP1	LDA (STRADR),Y	; UEBERTRAGEN
	...
	JSR CORR	; STRING ADRESSE IN DESCRIPTOR AENDERN
L3	CLC
	JMP BEGIN2	; NAECHSTER STRING
WEITER	LDA BUFPTR	; BUFFER LEER?
	...
	STY $59
	JSR $A3BF	; BASIC ROUTINE BLOCK VERSCHIEBEN
L6	LDA FLAG	; ENDFLAG GESETZT?
	BNE ENDE
	JMP BEGIN1	; NÄCHSTEN BEREICH UNTERSUCHEN

ENDE	PLA		; URSPRUENGLICHER ZUSTAND
...
GETSA	BCC G1		; C=0 -> NAECHSTEN STRING
	...
	STA STAT
DSTCK	LDA STRDP	; NAECHSTEN DESCRIPTOR
	CLC
	ADC #3		; DESCRIPTOR-LAENGE
	STA STRDP
	LDY #0
	LDA (STRDP),Y	; LAENGE
	STA LEN
	...
	CMP STRDP
	BEQ VARS	; ENDE, DANN WEITER MIT VARS
	LDA LEN
	BEQ DSTCK	; LEERER STRING, DANN NAECHSTEN
	RTS

VARS	LDA VARTAB	; VARIABLEN ANFANG
	LDX VARTAB+1
	STA STRDP
	STX STRDP+1
	LDA STRDP
	...
	STY STAT
	BNE V1
VAR	CLC		; NAECHSTE VARIABLE
	LDA STRDP
	ADC #7		; VARIABLENLAENGE
	STA STRDP
	BCC V1
	INC STRDP+1
...
G2	LDY #0		; VARIABLENNAME
	...
	BMI VAR		; KEIN STRING, NAECHSTE V.
	INY
	LDA (STRDP),Y
	BPL VAR		; KEIN STRING, NAECHSTE V.
	INY
	LDA (STRDP),Y	; STRINGLÄNGE
	BEQ VAR		; = 0, NAECHSTE V.
	STA LEN
	INY
...
A1	STA STRDP	; C=0
	...
	PHP		; VAR-TYP MERKEN
	INY
	LDA (STRDP),Y	; ZUM NAECHSTEN ARRAY
...
ARRAY	CLC
	LDA STRDP
	ADC #3		; STRING DESCIRPTOR LAENGE
	STA STRDP
	BCC A5
	INC STRDP+1
A5	LDX STRDP+1
	...
	BEQ NEXTARR
A6	LDY #0
	LDA (STRDP),Y	; STR. LAENGE
	BEQ ARRAY	; WEITER IM ARRAY
	STA LEN
	INY
	LDA (STRDP),Y	; ADREDSSE
	TAX
	INY
	LDA (STRDP),Y	; ADRESSE HIGH
	TAY
...
CORR	LDA STAT	; STR.ADR. KORRIGIEREN
	...
	AND #3		; NUR 2 BITS
	TAY
	LDA STRADR	; LAGE DES DESCRIPTOR
...
SPTR	!byte 0,0,0,0,0	; 5 BYTE ZERO PAGE SAVE
