...
; ******** Source: jk-gc.asm
   ...
   475                          
   476  c61d adc4c7             	LDA ORIGVID	; Kontrollanzeige löschen
   ...
   499                          }
   500  c632 38                 	SEC
   501  c633 24                 	!byte $24	; BIT ZP, d.h. nächsten Befehl ignorieren!
   502                          NEXTSTR	
   503  c634 18                 	CLC
   504                          NEXTSTR1
   505  c635 20a7c6             	JSR GETSA	; Nächste String-Adresse holen.
   ...
   567  c67f f023               	BEQ NOCOPY	; ist der Puffer leer, ev. nächster
   568                          +			; Bereich ...
   569                          } else {
   ...
   608  c698 48                 	PHA		; damit das Puffer-RAM zugänglich
   ...
   616                          			; Z=1
   617  c6a0 68                 	PLA		; ursprünglicher Zustand
   618  c6a1 8501               	STA PROZPORT	; KERNAL-ROM wieder aktivieren
   619  c6a3 58                 	CLI
   620                          NOCOPY
   621  c6a4 4ce4c5             	JMP NEXTBLOCK	; nächsten Bereich
   622                          
   623                          
   624                          ;
   ...
   633                          ; die Stringlänge.
   634                          
   635  c6a7 9063               GETSA	BCC CHECKTYPE	; C=0 -> nächsten String laut STAT
   ...
   658  c6b7 e8                 NEXTDST	INX		; nächster Descriptor
   659  c6b8 e8                 	INX
   660  c6b9 e8                 	INX
   661                          ISDSTEND
   662  c6ba e416               	CPX TSSP	; Stack durch?
   663  c6bc f010               	BEQ VARS
   664  c6be b500               	LDA 0,X		; String-Länge
   665  c6c0 f0f5               	BEQ NEXTDST
   666  c6c2 8552               	STA LEN		; Rückgabevariable
   ...
   684                          VAR
   685  c6dc 18                 NEXTVAR	CLC		; nächste Variable
   686  c6dd a522               	LDA STRDP
   687  c6df 6907               	ADC #7		; Variablenlänge
   688  c6e1 8522               	STA STRDP
   689  c6e3 9002               	BCC ISVAREND
   690  c6e5 e623               	INC STRDP+1	; Überlauf High-Byte
   691                          ISVAREND
   ...
   700  c6f5 30e5               	BMI NEXTVAR	; kein String, nächste V.
   701  c6f7 c8                 	INY
   702  c6f8 b122               	LDA (STRDP),Y	; 2. Zeichen, Typ in Bit 7
   703  c6fa 10e0               	BPL NEXTVAR	; kein String, nächste V.
   704  c6fc c8                 	INY
   705  c6fd b122               	LDA (STRDP),Y	; String-Länge
   706  c6ff f0db               	BEQ NEXTVAR	; = 0, Nächste Variable
   707  c701 8552               	STA LEN		; Rückgabevariable
   708  c703 c8                 	INY
   ...
   745  c738 c8                 	INY
   746  c739 b122               	LDA (STRDP),Y	; Offset nächstes Array
   ...
   764                          NEXTASTR
   765  c754 18                 	CLC
   766  c755 a903               	LDA #3		; String-Descriptor-Länge
   767  c757 6522               ADVDESC	ADC STRDP	; nächten String
   768  c759 8522               	STA STRDP
   769  c75b 9002               	BCC +
   770  c75d e623               	INC STRDP+1	; Überlauf High-Byte
   771  c75f c550               +	CMP PTR		; Array durch?
   772  c761 d006               	BNE IS0ASTR
   773  c763 a623               	LDX STRDP+1
   774  c765 e451               	CPX PTR+1
   775  c767 f0ba               	BEQ CHKAEND	; A/X = PTR, Array-Ende prüfen
   776                          IS0ASTR
   777  c769 a000               	LDY #0
   778  c76b b122               	LDA (STRDP),Y	; String-Länge
   779  c76d f0e5               	BEQ NEXTASTR	; weiter im Array
   780  c76f 8552               	STA LEN		; Rückgabevariable
   781  c771 c8                 	INY
   ...
   833  c793 a8                         TAY           ; Länge low
   834  c794 a55b                       LDA $5B       ; Endadresse high
   835  c796 e560                       SBC $60       ; minus Startadresse high
   836  c798 aa                         TAX           ; Länge high, als DEC-DEC-Counter
   837  c799 98                         TYA           ; Länge low
   838  c79a f019                       BEQ copy      ; wenn 0, dann geht es einfach ...
   839  c79c 18                         CLC           ; Länge in A
   840  c79d 655f                       ADC $5F       ; Quellstartadresse um Low-Byte-Offset
   841  c79f 855f                       STA $5F       ; korrigieren: -(-Länge) -> +Länge
   842  c7a1 b002                       BCS +         ; da eigentlich Subtraktion, Überlauf
   843  c7a3 c660                       DEC $60       ; entspr. in Startadresse high korr.
   844                          +
   845  c7a5 98                         TYA           ; Länge low
   846  c7a6 18                         CLC
   847  c7a7 6558                       ADC $58       ; Zielstartadresse um Low-Bye-Offset
   848  c7a9 8558                       STA $58       ; korrigieren: -(-Länge) -> +Länge
   849  c7ab b002                       BCS +         ; da eigentlich Subtraktion, Überlauf
   850  c7ad c659                       DEC $59       ; entspr. in Startadresse high korr.
   851                          +
   852  c7af e8                 	INX           ; Page-Zähler (alle ganzen und einen teilweise)
   853  c7b0 98                 	TYA           ; Länge low
